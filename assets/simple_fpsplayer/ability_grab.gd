...
func _physics_process(delta):
	if get_collider() is VehicleBody3D:
	...
	if Input.is_action_just_pressed("car_interact"):
		if can_use:
			...
			if not object_grabbed:
				...
				if get_collider() is VehicleBody3D:
					get_collider().freeze = false
					get_collider().get_node("look/Camera3D").current = true
				
					get_collider().entered_car()
					print("i exist")
			else:
				release()
	else:
